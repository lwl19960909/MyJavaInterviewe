...
static可以修饰变量，表示声明该变量为静态变量，即无论多少个实例化对象，都只有一份静态变量的拷贝，另外static不能修饰局部变量。

static可以修饰方法，表示该方法是独立于对象的静态方法，即无需实例化类对象也可调用。注静态方法不能使用类的非静态变量。



final变量表示该变量一旦被初始化就无法重新赋值。

final方法表示该方法可以被子类继承，但不可被子类重写。
...
1. 使用final修饰类
2. 只有私有构造函数的类
3. 没有声明为public的类只能被同包的类继承，不能被外包类继承
...
java实现一个对象的序列化可以通过实现Serializable接口或者Externalizable接口。对于Serializable接口，不想序列化的字段可以用transient修饰。

因为Serializable对象完全以它存储的二进制来构造，因此无需调用任何构造函数，但是**如果实现Serializable接口的类的父类没有实现Serializable接口时，反序列化过程会调用父类的默认构造函数，因此父类必须要有默认构造函数，不然会反序列化失败。**

其中writeObject方法和readObject方法可以控制如何序列化各属性，一次来实现不序列化和加密序列化。

对于transient修饰的属性会隔离在序列化之外，反序列化时就无法获得这个属性了。

Externalizable接口时Serializable的子接口，用户需要实现wiriteExternal和readRxternal来决定如何实现序列化和反序列化，因为序列化过程需要自己实现，所以无需使用transient。在对Externalizable进行反序列化时，会调用类的无参构造方法，要求要有一个public的无参构造方法否则会序列化失败。

序列化和反序列化如果前后版本不同也会失败。

## 11、ArrayList和LinkedList的区别和底层实现？如何实现线程安全？ 
ArrayList底层是基于数组的，LinkedList底层是双向链表。
...
fail-fast:fail-fast是java集合的一种错误检测机制，当多个线程对集合进行**结构上的改变**操作操作是，有可能会产生。(这个机制由迭代器产生，在迭代期间，如果结构发生改变可能会导致异常。)其产生的根本原因是因为其他线程对结构的改变导致了expectModCount和modCount不一致。一般的解决方法可以是加锁，或者使用copyOnWriteArrayList

fail-safe:会复制一个原来的集合，然后再复制的副本上进行遍历。

## 13、详细介绍HashMap。角度：数据结构+扩容情况+put查找的详细过程+哈希函数+容量为什么始终都是2^N+JDK1.7与JDK1.8的区别。 
数据结构：

1.7版本hashmap采用数组＋链表实现。1.8版本数组+链表+红黑树。

扩容：

默认容量是16，当数组达到负载阈值，会触发扩容，扩容的方式2倍扩容，原因是因为hash的计算方法是：newHash = oldHash & (length-1)，比如原有长度是8

老hash  ：10|01001

新的长度    00|11111

因为hash的计算是按位与length-1，length是2^n的话length-1就是11111...1&x=x，可以最大程度维持原来的hash不变。

当链表长度达到8时就会触发修改链表为红黑树的条件。

查找过程：

首先会计算出当前key的hash，然后查询数组对应偏移量的位置是否为空，就封装成entry存入数组，如果发生冲突就遍历链表或者查找红黑树，如果已经存在，就更新val，不存在就封装成entry插入。

其他区别：

1.7在扩容rehash的时候采用的是头插法，会造成死循环，1.8的时候修改为尾插法，解决了这一问题。



另外可以对比hashmap,treemap,hashtable

相比于treemap主要的区别是底层的实现treemap是基于红黑树的。

hashtable是线程安全的，kv都不能为null，实现线程安全的方式是锁住整个hashtable。

## 14、HashMap如何实现线程安全？ConcurrentHashMap的底层实现？JDK1.7与JDK1.8的区别 
hash实现线程安全的方式：插入时使用synchronized加锁。

concurrentHashMap：

数据结构：

1.7版本采用数组+链表的方式，1.8也变成是数组+链表+红黑树。

1.7的形式是Segment数组（默认16）+HashEntry数组+HashEntry链表的形式

1.8的数据结构和hashmap一致。

查找：

1.7时要进行两次hash去定位，第一次获取Segment第二次获取kv，1.8只需要一次。

线程安全：

1.7时加锁对象是Segment数组对应偏移量的对象，由于Segment还维持一个HashEntry数组+链表，加锁粒度相对较大，此时采用ReentrantLock。

1.8抛弃了Segment的概念，加锁对象是HashEntry数组指定偏移量的的对象，加锁粒度相对较小，采用CAS+Synchornized的方式加锁。
...
4. 还在扩容就先扩容
5. 链表长度超过8就先转红黑树在插入
6. 添加成功就addcount统计size

使用synchornized代替Reentrantlock的原因：

1. 锁粒度降低，Reentrantlock相比之下优势不明显。
2. 大量数据下API的Reentrantlock会有更大的开销
3. JVM从未放弃对Synchornized的优化。

## 15、正则表达式会写吗？ 

## 16、设计模式了解吗？ 
里氏替换原则，开闭原则，依赖倒置原则，接口隔离原则，最少知道原则，合成复用原则
...
线程独占区：虚拟机栈，指令计数器，本地方法栈

其中方法区是一种逻辑概念的标准，不同的虚拟机对方法区的实现不同，比如1.7的永久代到1.8的元空间。

要**注意分别存储什么东西**：

堆：类的实例对象

元空间：类的结构信息，编译后字节码，静态变量，常量，存在GC，主要回收两种，一种是不再被使用的常量和不再被使用的类模板（堆中没有任何该类的示例，没有任何对该类的class的引用，该类的类加载器已被回收）

虚拟机栈：虚拟机栈由栈帧组成，栈帧内部包括局部变量表，动态链接，操作数栈，返回地址。

指令计数器（PC寄存器）：指向方法区中下一条指令的字节码
...
元空间取代永久代，元空间使用本地内存，永久代使用堆内存。

元空间的常量池里，字符串常量池被放在了堆，运行时常量池被放在了元空间。
...
伊甸园区：from区：to区=8:1:1



堆内存分配策略：

1. 对象优先分配在伊甸园区，如果伊甸园区没有足够的空间，就触发一次MinorGC
2. 大对象直接分在老年代（需要大量连续空间的对象），避免新生代由于复制算法产生的大量内存拷贝
3. 空间分配担保：每次进行MinorGC时，JVM会计算转移到老年代对象的平均大小，如果大于剩余空间就触发一次full GC。 如果小于检查HandlePromotionFailure设置，如果true则只进行Monitor GC,如果false则进行Full GC。 （-XX：HandlePromotionFailure）
4. 设置新生代对象年龄，达到阈值（默认15）就进入老年代
5. 动态判断对象的年龄，如果survivor区中所有相同年龄的对象的总大小大于survivor空间的一半，那么 大于等于该年龄的对象可以直接进入老年代。

## 3、Full GC触发条件 
1. 新生代对象晋升老年代空间不足
2. 触发MinorGC时，HandlePromotionFaliure参数设置为false

## 4、如何判断对象是否存活？回收对象的两次标记过程。 
引用计数，无法解决循环引用

可达性分析，根据引用关系构成一个有向图，从根节点（局部变量引用的对象，静态引用的对象，常量引用的对象，JNI方法引用的对象）出发做可达性分析。

第一次标记直接与根节点相连的，第二次做可达性分析

（也可能是第一次标记直接相连的，并发追踪，第二次标记在并发过程中发生改变的）

## 5、垃圾回收算法以及垃圾回收器介绍，尤其是G1和CMS的优缺点 
垃圾回收算法：

1. 复制算法（主要是用在新生代）：将伊甸园区，from区或者的对象复制到to区并清空伊甸园区和from区。优点：没有碎片，缺点复制开销。
2. 标记清除：标记所有待清除对象，集中清理。优点：不需要占用额外空间，缺点有碎片，两次扫描花时间
3. 标记整理算法：对标记清除添加一个压缩的步骤。优点：没碎片，缺点：复制开销

垃圾回收器：

新生代：Serial（串行回收器），ParNew（并行回收器），Parallel Scavenge(并行回收器，相比较于ParNew，它关注可控的吞吐量，可以动态调整参数，以达到提供合适停顿时间或最大吞吐量的问题)

老年代：Parallel Old

CMS(并发标记清除)初始标记（STW，标记GC root直连节点）-》并发追踪-》重新标记（STW）-》并发清除

相比较于传统算法，停顿时间短，但本身基于标记清除算法，会产生碎片

G1：初始标记，并发标记，最终标记，筛选回收（根据时间进行最大化回收）优点，停顿时间可控，没有碎片，缺点：复制开销
...
双亲委派：所有的类加载都先交由父类加载器加载。

好处：防止恶意篡改系统类造成问题

破坏双亲委派机制：编写自定义类加载器，继承ClassLoader类，重写findclass和loadclass方法
